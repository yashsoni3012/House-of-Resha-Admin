...
import { showBlogUpdated } from "../utils/sweetAlertConfig";
...
export default function EditBlog() {
  ...
  const [deletedContentImages, setDeletedContentImages] = useState([]); // Array of indices
  ...
  const fetchBlogData = async () => {
    try {
      ...
      if (blog.content) {
        ...
        setExistingImages((prev) => ({ ...prev, content: [] }));
      }
    } catch (error) {
  ...
  const handleCoverImageChange = (e) => {
    ...
    if (file) {
      ...
      // Clear deletion state since we're uploading a new image
      setDeletedCoverImage(false);
    }
  };
  ...
  const handleContentImageChange = (index, file) => {
    ...
    setContentPreviews((prev) => {
      ...
      if (file) {
        ...
        // Remove from deleted images if re-uploading
        setDeletedContentImages((prevDeleted) =>
          prevDeleted.filter((i) => i !== index)
        );
      } else {
  ...
  const handleRemoveCoverImage = () => {
    setShowCoverDeleteConfirm(true);
  };
  ...
  const confirmRemoveContentImage = () => {
    ...
    // Mark this content image for deletion
    setDeletedContentImages((prev) => {
  ...
  const removeContentBlock = (index) => {
    ...
    // Remove from deleted images if this index was marked for deletion
    setDeletedContentImages((prev) => prev.filter((i) => i !== index));
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (formData.coverImage) {
        ...
        // User explicitly deleted old cover
        formDataToSend.append("removeCover", "true");
      }
      // If no new cover and not deleted, the existing cover will remain

      // Prepare content payload and handle content images
      ...
      console.log("Deleted cover:", deletedCoverImage);
      console.log("Deleted content images:", deletedContentImages);
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {deletedCoverImage && (
                    <p className="text-xs text-red-600 mt-1">
                      ✓ Cover image marked for deletion
                    </p>
                  )}
                  {existingImages.cover && !deletedCoverImage && (
                ...
                {coverPreview ? (
                  ...
                  <label className="flex flex-col items-center cursor-pointer">
                    ...
                    <p className="text-gray-800 font-semibold text-sm sm:text-base mb-1 text-center">
                      {deletedCoverImage
                        ? "Cover image deleted - Upload new image"
                    ...
                    <div className="px-4 sm:px-5 md:px-6 py-2 sm:py-2.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors font-medium text-sm sm:text-base">
                      {deletedCoverImage || !existingImages.cover
                        ? "Choose File"
                        : "Replace File"}
                    </div>
                    <input
                ...
                >
                  <div className="flex items-center justify-between mb-4 sm:mb-6">
                    <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                      ...
                      {deletedContentImages.includes(index) && (
                        <span className="text-xs bg-gradient-to-r from-red-100 to-red-200 text-red-800 px-2.5 py-1 rounded-full font-medium shadow-sm">
                          Image Deleted
                        </span>
                      )}
                    </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-lg p-6 max-w-sm w-full shadow-xl">
            ...
            <p className="text-sm text-gray-600 mb-6">
              ...
              the current cover image from the server.
            </p>
            <div className="flex justify-end gap-3">
              ...
              <button
                type="button"
                onClick={confirmRemoveCoverImage}
